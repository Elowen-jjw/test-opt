...
public class LoopExecValues {
	...
	public static int getTimes(File file, List<String> initialList, int startLine, int endLine) {
		try {
			...
			String command = (PropertiesInfo.compilerType.equals("gcc")? "export LANGUAGE=en && export LANG=en_US.UTF-8 && ": "")
					+ "cd " + file.getParent() + " && " + PropertiesInfo.compilerType + " " + newExecFileName
			...
			List<String> execLines = ProcessTerminal.listMemCheck(command, 20, "sh", false, true, new ArrayList<>(Arrays.asList(aoutFilename)));
	...
	public static boolean checkConsistency(File file, List<String> initialList, int startLine, int endLine) {
		try {
			...
			List<String> execLines = ProcessTerminal.listMemCheck(command, 30, "sh", false, true, new ArrayList<>(Arrays.asList(aoutFilename)));
