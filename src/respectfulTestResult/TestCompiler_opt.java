...
public class TestCompiler_opt {
    ...
    public TestCompiler_opt(File muDir, String compilerType){
        this.muDir = muDir;
...
//        }
        ...
        irInput = random.nextBoolean();
        irOutput = random.nextBoolean();
    ...
    public String getRandomConfig(){
        ...
        if(compilerType.equals("gcc")){
            configList = gccConfigList;
        }else if(compilerType.equals("llvm")){
            configList = llvmConfigList;
        }
        StringBuilder result = new StringBuilder();
        Set<String> configSet = new TreeSet<>();
        Random random = new Random();
        int number = random.nextInt(1, configList.size());
        for(int i=0; i<number; i++){
            configSet.add(configList.get(random.nextInt(configList.size())));
        }
    ...
    public CompilationInfo runLlvm(File file){
        ...
        Map<String,List<String>> outputListMap = new TreeMap<>();
        String aoutFilename = simplifyFilename(file);
        String trueFilename = file.getName().substring(0, file.getName().lastIndexOf(".c"));

        String inputFilename = trueFilename + (irInput? ".ll": ".bc");
        String outputFilename = trueFilename + "_opt" + (irOutput? ".ll": ".bc");

        for(String oClang: oList){
            String commandFront = "cd " + file.getParent() + " && clang " + file.getName() + " " + oClang
                    + " " + (oClang.equals("-O0")?"": config) + " -emit-llvm -I $CSMITH_HOME/include " + (irInput? "-S": "-c");
            System.out.println(commandFront);
            justProcess(commandFront);
            for(String oOpt: oList){
                String opCom = (oClang + "+" + oOpt).replaceAll("-", "");
                String commandOpt = "cd " + file.getParent() + " && opt " + inputFilename + " " + oOpt + " " + (irOutput? "-S": "") + " -o " + outputFilename;
                justProcess(commandOpt);

                String commandBack = "cd " + file.getParent() + " && clang " + outputFilename + " -o " + aoutFilename;
                justProcess(commandBack);

                String comandExec = "cd " + file.getParent() + " && ./" + aoutFilename;
                long executionTime = 0;
                File aoutFile = new File(file.getParent() + "/" + aoutFilename);
                if(!aoutFile.exists()) {
                    outputListMap.put(opCom, new ArrayList<>(List.of("error")));//没有生成a.out的标记成error
                }
                else{
                    long startTime = System.currentTimeMillis();
                    ...
                    executionTime = System.currentTimeMillis() - startTime;
                    deleteAoutFile(file, aoutFilename);
                    outputListMap.put(opCom, analysisOutputResult(execLines));
                }
                File optFile = new File(file.getParent() + "/" + outputFilename);
                optFile.delete();
                performanceMap.put(opCom, String.valueOf(executionTime));
                System.out.println(opCom + "   " + executionTime + "   " + outputListMap.get(opCom));
            }

            File irFile = new File(file.getParent() + "/" + inputFilename);
            irFile.delete();
        }
        return new CompilationInfo(outputListMap, performanceMap);
    }
